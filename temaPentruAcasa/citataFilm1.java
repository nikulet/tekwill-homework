package Data17marti.temaPentruAcasa;

public class citataFilm1 {
    public static void main(String[] args) {

        System.out.println("Ex 1\n");

        System.out.println("De \"miliarde\" era tipul,\n" +
                "Spre-navuţire-ncrâncenat,\n" +
                "Da-n filmul vieţii lui văd \"clipul\",\n" +
                "Când fu şi el... \"denominat\".");

//***************************************************8
        System.out.println("***************************************************\n");
        System.out.println("Ex 2\n");
        System.out.println("365 дней\n" +
                "Жанр\tэротическая драма\n" +
                "Режиссёр\tБарбара Бяловач\n" +
                "Томаш Мандес\n" +
                "Автор\n" +
                "сценария\tТомаш Клималла\n" +
                "Барбара Бяловач\n" +
                "Томаш Мандес\n" +
                "Бланка Липинска\n" +
                "В главных\n" +
                "ролях\tМикеле Морроне\n" +
                "Анна-Мария Секлуцка\n" +
                "Композитор\tМихал Сарапата\n" +
                "Матеуш Сарапата\n"+
                "Год	2020");

        System.out.println("*************************************************\n");

        System.out.println("Ex 3\n");
        System.out.println("x                      x");
        System.out.println("x                      x");
        System.out.println("x      xxxxxxxxxx      x");
        System.out.println("xxxxx  x        x  xxxxx");
        System.out.println("x   x  x        x  x   x");
        System.out.println("x   x  x        x  x   x");






    }
}
